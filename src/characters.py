...
class StArcher:
    ...
    def deal_damage(self, hit):
        ...
        # Check if time elapsed since spawn is greater than 10 seconds
        if current_time - self.spawn_time > 10:
            if not self.alive:
                return
            self.health -= hit
            if self.health <= 0:
...
class Healer:
    ...
    def heal_troop(self, x, y, King):
        target = None
        # if(V.map[x][y] == pt.TOWNHALL):
        #     target = V.town_hall_obj
        # else:
        if( King.position[0] == x and King.position[1] == y):
            target = King 
            self.heal_target(target)

        all_troops = collections.ChainMap( archers, barbarians, starchers, dragons, balloons)
        for troop in all_troops:
            if( troop.position[0] == x and troop.position[1] == y):
                target = troop
                # target = all_troops[(x, y)]
                self.heal_target(target)

    def heal_target(self, target):
        if(self.alive == False):
            return
        if ( abs(target.position[0] - self.position[0]) + abs(target.position[1] - self.position[1]) <= self.heal_radius):
...
def move_healers(King, V):
    for bal in healers:
        if(bal.alive == False):
            continue
        closest_troop = search_for_closest_troop(bal.position, V, King)
        if(closest_troop == None):
            continue

        bal.move(closest_troop, V)
        bal.heal_troop(closest_troop[0], closest_troop[1], King)
...
def search_for_closest_troop(pos, V, King):
    closest_troop = None
    closest_dist = 800085
    flag = 0
    ...
    distance = abs(King.position[0] - pos[0]) + abs(King.position[1] - pos[1])
    if( distance <= closest_dist and King.health < King.max_health):
        closest_dist = distance
        closest_troop = (King.position[0], King.position[1])
        

    all_troops = collections.ChainMap(archers, barbarians, starchers, dragons, balloons)
    for troops in all_troops:
        distance = abs( pos[0] - troops.position[0]) + abs( pos[1] - troops.position[1])
        if( distance <= closest_dist and troops.health < troops.max_health):
            closest_dist = distance
            closest_troop = (troops.position[0], troops.position[1])
