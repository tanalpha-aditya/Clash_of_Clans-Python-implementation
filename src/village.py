...
class Village:
    ...
    def remove_wall(self, wall_obj):
        position = wall_obj.position
        blast_radius = wall_obj.attack_radius

            # Check if the wall level is greater than 3, then trigger blast
        if wall_obj.level > 3:
            # Blast nearby troops (barbarians and archers)
            troops = barbarians + archers + starchers
            for troop in troops:
                if (troop.position[0] - position[0])**2 + (troop.position[1] - position[1])**2 <= blast_radius**2:
                    troop.health -= wall_obj.attack
                    if troop.health <=0 : troop.kill()
            for king in King.obj:
                if(king.position[0] - position[0])**2 + (king.position[1] - position[1])**2 <= blast_radius**2:
                    king.health -= wall_obj.attack
                    if king.health <=0 : king.kill()
            for king in Queen.obj:
                if(king.position[0] - position[0])**2 + (king.position[1] - position[1])**2 <= blast_radius**2:
                    king.health -= wall_obj.attack
                    if king.health <=0 : king.kill()
        self.wall_objs.pop(position)
        self.map[position[0]][position[1]] = pt.BLANK 
